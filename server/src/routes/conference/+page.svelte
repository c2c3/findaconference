...
<article class="w-full h-full">
	<div class="px-6 py-5 text-left h-full">
		<h1 class="text-[32px] mb-4">Add a Conference</h1>
        {#if form?.result}
	<!-- this message is ephemeral; it exists because the page was rendered in
		   response to a form submission. it will vanish if the user reloads -->
	<a class="h-12 border-black border-2 p-2.5 bg-yellow-200 hover:bg-yellow-300 hover:shadow-[2px_2px_0px_rgba(0,0,0,1)] active:bg-yellow-400" href={`/conference/${form.result}`}>Successfully created a new conference! Click here to go to conference details!</a>
{/if}
		<form method="POST">
